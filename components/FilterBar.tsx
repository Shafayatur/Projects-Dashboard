...
export default function FilterBar({
  ...
  onProductTypeChange: (t: string) => void;
}) {
  return (
    <div className="border-2 border-line p-4 mb-8 flex flex-wrap gap-6 items-start">
          ...
          {platforms.map((p) => {
            ...
            return (
              <button
                ...
                className={`px-3 py-1.5 text-xs font-bold border-2 border-line transition-colors ${
                  active ? "bg-ink text-black" : "bg-black text-ink hover:bg-white/10"
                }`}
              >
                {labels[p] || p}
              </button>
      ...
      {productTypes.length > 0 && (
        <div>
          <div className="text-xs uppercase tracking-wide text-muted font-bold mb-2">Product type</div>
          <select
